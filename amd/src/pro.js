...
define(['jquery', 'core/log', 'core/config'], function($, log, cfg) {
    ...
    var moodlescripttoolpro = {

        init: function() {

            $('#id_s_tool_moodlescript_licensekey').bind('change', this.check_product_key);
            $('#id_s_tool_moodlescript_licensekey').trigger('change');
            log.debug('AMD Pro js initialized for tool moodlescript');
        },
        ...
        check_product_key: function() {
            ...
            var calculated = moodlescripttoolpro.checksum(payload);
            ...
            if (crc === calculated) {
                var url = cfg.wwwroot + '/admin/tool/moodlescript/pro/ajax/services.php?';
                url += 'what=license';
                url += '&service=check';
                url += '&customerkey=' + that.val();
                url += '&provider=' + $('#id_s_tool_moodlescript_licenseprovider').val();

                $('#id_s_tool_moodlescript_licensekey + img').remove();
                $('#id_s_tool_moodlescript_licensekey').after(waiticon);

                $.get(url, function(data) {
                    if (data.match(/SET OK/)) {
                        $('#id_s_tool_moodlescript_licensekey + img').remove();
                        $('#id_s_tool_moodlescript_licensekey').after(validicon);
                    } else {
                        $('#id_s_tool_moodlescript_licensekey + img').remove();
                        $('#id_s_tool_moodlescript_licensekey').after(invalidicon);
                    }
                }, 'html');
            } else {
                $('#id_s_tool_moodlescript_licensekey + img').remove();
                $('#id_s_tool_moodlescript_licensekey').after(cautionicon);
            }
        },
